...
export const HealthModule: React.FC = () => {
  // 1. 初始化状态：深度清理逻辑，防止坏数据导致白屏
  const [records, setRecords] = useState<HealthRecord[]>(() => {
    try {
      const saved = appStorage.get('health-records', []);
      // 强制检查：必须是数组，且每一项都必须是有效的对象并含有日期
      if (Array.isArray(saved)) {
        return saved.filter(r => r && typeof r === 'object' && r.date);
      }
      return [];
    } catch (e) {
      console.error("初始化数据失败，重置为空数组", e);
      return [];
    }
  });
  ...
  // 2. 自动保存：确保 records 永远不会变成 undefined
  useEffect(() => {
    appStorage.set('health-records', records || []);
  }, [records]);

  // 3. 保存记录逻辑：增加了 records 存在性保护
  const handleSaveRecord = () => {
    ...
    if (existingIndex >= 0) {
      ...
      const newList = [newRecord, ...currentRecords].sort((a, b) => 
  ...
  const getStats = () => {
    ...
    const weightRecords = filteredData.filter(r => r && r.weight);
    ...
    const bpRecords = filteredData.filter(r => r && r.bloodPressure);
    const latestBP = bpRecords.length > 0 ? bpRecords[bpRecords.length - 1].bloodPressure : null;
    const bpWarning = latestBP ? (latestBP.systolic >= 140 || latestBP.diastolic >= 90) : false;

    const sleepRecords = filteredData.filter(r => r && r.sleepHours !== undefined);
    const avgSleep = sleepRecords.length > 0 
      ? sleepRecords.reduce((sum, r) => sum + (r.sleepHours || 0), 0) / sleepRecords.length 
      : 0;

    const waterRecords = filteredData.filter(r => r && r.waterIntake !== undefined);
    const avgWater = waterRecords.length > 0 
      ? waterRecords.reduce((sum, r) => sum + (r.waterIntake || 0), 0) / waterRecords.length 
      : 0;
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                <Line type="monotone" dataKey="weight" stroke="#7B2CBF" strokeWidth={2} name="体重" connectNulls />
                <Line type="monotone" dataKey="systolic" stroke="#FFD700" strokeWidth={2} name="收缩压" connectNulls />
                <Line type="monotone" dataKey="sleep" stroke="#4CC9F0" strokeWidth={2} name="睡眠" connectNulls />
              </LineChart>
            </ResponsiveContainer>
          ) : (
